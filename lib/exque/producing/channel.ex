defmodule Exque.Producing.Channel do
  ...
  defp publish(exchange_name, struct, channel) when is_atom(exchange_name) do
    publish(Atom.to_string(exchange_name), struct, channel)
  end
  ...
  defp publish(exchange_name, struct, channel) do
    AMQP.Basic.publish(
      ...
      [
        ...
        app_id: struct.metadata.app,
        type: struct.message_type,
        timestamp: Timex.to_unix(struct.metadata.created_at),
        persistent: true,
        priority: 0
      ]
    )
